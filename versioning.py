# This file is adapted from http://docs.sqlalchemy.org/en/latest/_modules/examples/versioned_history/history_meta.html
# which gives an example of how to create versioned shadow tables in SQLAlchemy.
# This file is a verbatim copy, except for the parts marked "### CHANGED ###".
# The goal of the changes is to adjust which columns are added to the primary and shadow tables.
...
from sqlalchemy import event, util, text
...
def get_primary_columns():
    return [
        Column('sys_period', TSTZRANGE, nullable=False, server_default=text("tstzrange(now(), null)"), info={"version_meta": True})
    ]

def get_history_columns():
    return [
        Column('sys_period', TSTZRANGE, nullable=False, server_default=text("tstzrange(now(), null)"), info={"version_meta": True}, primary_key=True)
    ]
...
def _history_mapper(local_mapper):
    ...
    if not super_mapper or \
        ...
        ### CHANGED ###
        cols.extend(get_history_columns())
    ...
    if not super_history_mapper:
        ### CHANGED ###
        for add_col in get_primary_columns():
            local_mapper.local_table.append_column(add_col)
