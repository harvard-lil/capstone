...
function addMapMouseovers() {
    var svgmap = document.getElementById("usa_territories_white");
    ...
    svgmap.childNodes.forEach(function (element) {
        var map_data = {
        ...
        element.addEventListener('mouseover', function (mouseover_event) {
            Array.from(document.getElementsByClassName("num_cases")).forEach(function (element) {
                element.innerHTML =
                    map_data[mouseover_event.target.id]['case_count'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            })
            Array.from(document.getElementsByClassName("num_reporters")).forEach(function (element) {
                element.innerHTML =
                    map_data[mouseover_event.target.id]['reporter_count']
            })
            Array.from(document.getElementsByClassName("num_pages")).forEach(function (element) {
                element.innerHTML =
                    map_data[mouseover_event.target.id]['page_count'].toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            })
        })
    })
}
