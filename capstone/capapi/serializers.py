...
class CitationWithCaseSerializer(CitationSerializer):
    case_url = serializers.HyperlinkedRelatedField(
        source="case", view_name="cases-detail", read_only=True, lookup_field="id"
    )
...
class JurisdictionSerializer(serializers.ModelSerializer):
    url = serializers.HyperlinkedIdentityField(
        view_name="jurisdiction-detail", lookup_field="slug"
    )
...
class CaseDocumentSerializer(BaseDocumentSerializer):
    ...
    def get_preview(self, instance):
        ...
        if "_source" in instance:
            preview_source = (
                [
                    highlight
                    for highlights in instance["highlight"].values()
                    for highlight in highlights
                ]
                if "highlight" in instance
                else []
            )
        if "inner_hits" in instance:
            for nested_field, nested_hits in instance["inner_hits"].items():
                ...
                _ = [
                    preview_inner.extend(list(highlight["highlight"].values()))
    ...
    def to_representation(self, instance):
        ...
        ec = [
            ...
            for o in s["casebody_data"]["text"]["opinions"]
    ...
    def data(self):
        ...
        with transaction.atomic(), transaction.atomic(using="user_data"):
            ...
            if not user.unlimited_access_in_effect():
                ...
                user.case_allowance_last_updated = (
                    refreshed_user.case_allowance_last_updated
                )
                user.has_tracked_history = refreshed_user.has_tracked_history
                user.update_case_allowance(
                    save=False
                )  # for SiteLimits, make sure we start with up-to-date case_allowance_remaining
            ...
            if user.has_tracked_history:
                if hasattr(self, "many"):
                    case_ids = [
                        case["_source"]["id"]
                        for case in self.instance
                        if case["_source"]["restricted"]
                    ]
                else:
                    case_ids = [self.instance["id"]]
                allowed_case_ids = set(
                    UserHistory.objects.filter(case_id__in=case_ids, user_id=user.id)
            ...
            if user.track_history:
                cases = (
                    result if hasattr(self, "many") else [result]
                )  # self.many means this is a list view
                to_create = [
                    UserHistory(user_id=user.id, case_id=c["id"])
                    for c in cases
                    if c["casebody"]["status"] == "ok"
                ]
                if to_create:
...
class CaseDocumentSerializerWithCasebody(CaseAllowanceMixin, CaseDocumentSerializer):
    ...
    def to_representation(self, instance, check_permissions=True):
        ...
        if not check_permissions:
            ...
            request.user.has_tracked_history
            and case["id"] in self.context["allowed_case_ids"]
        ):
            status = "ok"
        elif (
            request.site_limits.daily_downloads
            >= request.site_limits.daily_download_limit
        ):
        ...
        data = None
        if status == "ok":
            body_format = self.context.get(
                "force_body_format"
            ) or request.query_params.get("body_format")
...
class VolumeSerializer(serializers.ModelSerializer):
    ...
    reporter_url = serializers.HyperlinkedRelatedField(
        source="reporter", view_name="reporter-detail", read_only=True
    )
...
class CourtSerializer(serializers.ModelSerializer):
    ...
    url = serializers.HyperlinkedIdentityField(
        view_name="court-detail", lookup_field="slug"
    )
