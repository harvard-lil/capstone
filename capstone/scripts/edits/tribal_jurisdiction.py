...
def make_edits(dry_run=True):
    ...
    courts = Court.objects.filter(case_metadatas__reporter=reporter, jurisdiction=old_jurisdiction).distinct()
    ...
    for court in courts:
        ...
        if court.case_metadatas.values('reporter_id').distinct().count() > 1:
