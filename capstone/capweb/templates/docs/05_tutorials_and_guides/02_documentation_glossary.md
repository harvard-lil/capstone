...
data to another computer program. CAPAPI is a [RESTful]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-restful) API designed to distribute court case data.
{: class="mt-1" }


* Character
{: class="list-header mb-0" id="def-character" }
* A letter, number, space, or piece of punctuation. Multiple characters together make up a [string]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-string).
{: class="mt-1" }
...
because they indicate to the server that everything after them is a [parameter]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-parameter).
{: class="mt-1" }
...
* [curl](https://curl.haxx.se/) is a simple [command line]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-command-line) tool for retrieving data over the 
internet. It's similar to a web browser in that it will retrieve the contents of a [url]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-url), but it will dump the
...
* A RESTful [API]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-api) is based on [HTTP](https://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol), and makes use
...
* A [string]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-string) with [special characters]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-special-character) removed for ease of inclusion in a 
[url]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-url).
{: class="mt-1" }


* String
{: class="list-header mb-0" id="def-string" }
* A string, as a type of data, just means an arbitrary list (or string) of [characters]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-character). A word is a 
...
* For our purposes, a parameter is just a piece of data with a label that can be passed to an [endpoint]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-endpoint) 
in a web request.
{: class="mt-1" }
...
* The URL path begins with the slash after the [top-level domain]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-tld) and ends with the question mark that signals
 the beginning of the [parameters]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-parameter). It was originally intended to point to a file on the server's hard 
 drive, but these days it's just as likely to point to an application [endpoint]({% url 'docs' '05_tutorials_and_guides/02_documentation_glossary' %}#def-endpoint).
{: class="mt-1" }
